...
function EditPro ({ setTemp }) {
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/product/" + location.state.id).then((res) => {
      ...
      formik.initialValues.unity=(res.data.unity);
      formik.initialValues.image=(res.data.image);
  ...
  const formik = useFormik({
    ...
    validate,
    onSubmit: values => {
        axios.put("http://localhost:8000/api/product/" + location.state.id, values).then(() => {
          navigate("/maxsulotlar");
          setTemp((values) => values);
        });
      // }
